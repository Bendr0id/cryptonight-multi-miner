...
void cryptonight_hash_dumb(void* output, const void* input, struct cryptonight_ctx* ctx) {
	...
	for (i = 0; (i < ctx->scratchpadSize); i += INIT_SIZE_BYTE) {
		...
		memcpy(&ctx->scratchpad[i], ctx->text, INIT_SIZE_BYTE);
	}
	...
	for (i = 0; (i < ctx->iterations / 2); ++i) {
		...
		aesb_single_round(&ctx->scratchpad[j], ctx->c, ctx->a);
		xor_blocks_dst(ctx->c, ctx->b, &ctx->scratchpad[j]);
		/* Iteration 2 */
		mul_sum_xor_dst(ctx->c, ctx->a, &ctx->scratchpad[((uint32_t *)ctx->c)[0] & ctx->algoMask]); 
		/* Iteration 3 */
		j = ((uint32_t *)ctx->a)[0] & ctx->algoMask;
		aesb_single_round(&ctx->scratchpad[j], ctx->b, ctx->a);
		xor_blocks_dst(ctx->b, ctx->c, &ctx->scratchpad[j]);
		/* Iteration 4 */
		mul_sum_xor_dst(ctx->b, ctx->a, &ctx->scratchpad[((uint32_t *)ctx->b)[0] & ctx->algoMask]);
	}

	memcpy(ctx->text, ctx->state.init, INIT_SIZE_BYTE);
	oaes_key_import_data(ctx->aes_ctx, &ctx->state.hs.b[32], AES_KEY_SIZE);
	for (i = 0; (i < ctx->scratchpadSize); i += INIT_SIZE_BYTE) {
		xor_blocks(&ctx->text[0 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 0 * AES_BLOCK_SIZE]);
		aesb_pseudo_round_mut(&ctx->text[0 * AES_BLOCK_SIZE], ctx->aes_ctx->key->exp_data);
		xor_blocks(&ctx->text[1 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 1 * AES_BLOCK_SIZE]);
		aesb_pseudo_round_mut(&ctx->text[1 * AES_BLOCK_SIZE], ctx->aes_ctx->key->exp_data);
		xor_blocks(&ctx->text[2 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 2 * AES_BLOCK_SIZE]);
		aesb_pseudo_round_mut(&ctx->text[2 * AES_BLOCK_SIZE], ctx->aes_ctx->key->exp_data);
		xor_blocks(&ctx->text[3 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 3 * AES_BLOCK_SIZE]);
		aesb_pseudo_round_mut(&ctx->text[3 * AES_BLOCK_SIZE], ctx->aes_ctx->key->exp_data);
		xor_blocks(&ctx->text[4 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 4 * AES_BLOCK_SIZE]);
		aesb_pseudo_round_mut(&ctx->text[4 * AES_BLOCK_SIZE], ctx->aes_ctx->key->exp_data);
		xor_blocks(&ctx->text[5 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 5 * AES_BLOCK_SIZE]);
		aesb_pseudo_round_mut(&ctx->text[5 * AES_BLOCK_SIZE], ctx->aes_ctx->key->exp_data);
		xor_blocks(&ctx->text[6 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 6 * AES_BLOCK_SIZE]);
		aesb_pseudo_round_mut(&ctx->text[6 * AES_BLOCK_SIZE], ctx->aes_ctx->key->exp_data);
		xor_blocks(&ctx->text[7 * AES_BLOCK_SIZE], &ctx->scratchpad[i + 7 * AES_BLOCK_SIZE]);
...
void cryptonight_hash_aesni(void *restrict output, const void *restrict input, struct cryptonight_ctx *restrict ct0)
{
    ...
    uint8_t ExpandedKey[256] __attribute__((aligned(16)));
	...
	longoutput = (__m128i *)ctx->scratchpad;
	...
	for(i = 0; likely(i < ctx->iterations); i++)
	{
		__m128i c_x = _mm_load_si128((__m128i *)&ctx->scratchpad[a[0] & ctx->algoMask]);
		__m128i a_x = _mm_load_si128((__m128i *)a);
		uint64_t c[2];
		c_x = _mm_aesenc_si128(c_x, a_x);

		_mm_store_si128((__m128i *)c, c_x);
		__builtin_prefetch(&ctx->scratchpad[c[0] & ctx->algoMask], 0, 1);

		b_x = _mm_xor_si128(b_x, c_x);
		_mm_store_si128((__m128i *)&ctx->scratchpad[a[0] & ctx->algoMask], b_x);

		uint64_t *nextblock = (uint64_t *)&ctx->scratchpad[c[0] & ctx->algoMask];
		uint64_t b[2];
		b[0] = nextblock[0];
		b[1] = nextblock[1];
		...
		uint64_t *dst = (uint64_t*)&ctx->scratchpad[c[0] & ctx->algoMask];
		dst[0] = a[0];
		dst[1] = a[1];
		...
		a[1] ^= b[1];
		b_x = c_x;
		__builtin_prefetch(&ctx->scratchpad[a[0] & ctx->algoMask], 0, 3);
	}
...
struct cryptonight_ctx* cryptonight_ctx(AlgoConfig AlgoConfig){
	...
	scratchpad = calloc(1, sizeof(AlgoConfig.scratchpadSize));
#else
	...
	scratchpad = mmap(0, AlgoConfig.scratchpadSize, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_HUGETLB|MAP_POPULATE, 0, 0);
	if (scratchpad == MAP_FAILED)
		scratchpad = calloc(1, AlgoConfig.scratchpadSize);
	if (ret) {
		madvise(scratchpad, AlgoConfig.scratchpadSize, MADV_RANDOM|MADV_WILLNEED|MADV_HUGEPAGE);
		if (!geteuid())
			mlock(scratchpad, AlgoConfig.scratchpadSize);
	}
#endif

	ret->scratchpad = scratchpad;
